...
var conductor; var startLobby = false; var songIsPlay = false;
var homeAudio; var editAudio;
		...
		verifyLogin((data)=>{
			...
			menuTracker.push({
				...
				retrievePlaylist().then(()=>{
					...
					if(urlParams?.quiz){
						showQuizDetail(urlParams?.quiz);
					}else if(urlParams?.user){
						showUserDetail(urlParams?.user);
					}else if(urlParams?.list){
						showListDetail(urlParams?.list);
					}else if(urlParams?.search){
						homeSearch(urlParams?.search);
					}else if(urlParams?.feed) {
						homeFeed(urlParams?.feed);
					}
					else{
						processAjaxData(null,`/home`,{
							html:'',pageTitle:`Home - Just Bops & Quizzes`
...
function deleteQuiz(qID){
	...
	myHandler({
		...
		if(res.success){
			...
			saveActivity({
				userID: userID, type:'Deleted Quiz',
...
function deleteList(lID){
	...
	myHandler({
		...
		if(res.success){
			...
			saveActivity({
				userID: userID, type:'Deleted Playlist',
					...
					$('<div>',{class:'row'}).append([
						$('<div>',{class:'col'}).append([
							$('<div>',{class:'userDetail'}).append([
								...
								$('<span>',{class:'userDOB'}).text(userData.DOB)
							]),
							$('<div>',{class:'userDetail'}).append([
			...
			onChange:(data)=>{
				...
				myHandler({
					...
					$('#containerR').load('html/home_userMenu_userSection.html',()=>{
						...
						$('.userShapeElem').on('beatHit',(event)=>{
							anime({
						...
						function showUserDetail_Recent(){
							...
							_promises.push(
								getActivity({
									...
									console.log(res);
									let qpromises = [];
									for(let _a of res.data){
										let _aDetails = {};
										// if(isJSON(_a.details)){
										// 	_aDetails = JSON.parse(_a.details);
											
										// }
										_aDetails = (_a.details);
										qpromises.push(
											myHandler({
...
async function retrieveFollowerActivity(){
	...
	return myHandler({
		...
		let myFollowing = [];
		if(res.data.following && isJSON(res.data.following)){
			myFollowing = JSON.parse(res.data.following);
		}
...
async function addUserToCont(uData,cont){
	...
	if(isObject(uData.preferences)){
		uprefs = (uData.preferences); //object
	}
...
async function addListToCont(lData,cont){
	...
	if(isJSON(lData.quizzes)){
		_quizzes = JSON.parse(lData.quizzes); //array
	}
...
async function addActivityToCont(aData,cont){
	...
	return myHandler({type:'user',subType:'get',data:{
		...
		if(isJSON(aData.details)){
			aDetail = JSON.parse(aData.details);
		}
		// console.log(aDetail);
...
function addListToDOM(lData){
	...
	var lDataQuizzes = JSON.parse(lData.quizzes);
	var divData = $('<div>',{
	...
	$('.quiZ').on('beatHit',(event)=>{
		var currBeat = event.detail.beat;
		if(currBeat%2==0){
...
function addQuizToDOM(qData){
	...
	var hashtagData = "";
	var qDataTags = JSON.parse(qData.hashtags);
...
function loadEvents(){
	...
	$('.menuContainer .menuSearch .formBtn').on('click',(event)=>{
		event.preventDefault(); event.stopPropagation();
		event.stopImmediatePropagation();
		let _query = $('.menuSearch .inputBox').val();
		homeSearch(_query);
	});
	...
	$('.formRadioBox').on('click',(event)=>{
		...
		var radioBtn = $(_parent).children('.formRadioBtn');
	...
	$('.formCheckBox').on('click',(event)=>{
		...
		event.stopImmediatePropagation();
		var _parent = $(event.currentTarget).parent();
		var checkBtn = $(_parent).children('.formCheckBtn');
		checkBtn[0].checked = !checkBtn[0].checked;
	});
	...
	$('.dropImage').on('drop', (event)=>{
		...
		var thisForm = $(event.currentTarget).parents('form')[0];
	...
	$('.dropImageFile').on('change',(event)=>{
		...
		var ffiles = []; var errorInFile = false;
		var thisForm = $(event.currentTarget).parents('form')[0];
		var thisElem = $(thisForm).find('.dropImage')[0];
	...
	$('.dropSongFile').on('change',(event)=>{
		...
		var ffiles = []; var errorInFile = false;
		var thisForm = $(event.currentTarget).parents('form')[0];
		var thisElem = $(thisForm).find('.dropAudio')[0];
		if(event.currentTarget.files){
			for (var i = 0; i < 1; i++) {
				var file = event.currentTarget.files[i];
				if(!file.type.startsWith('audio/')){
	...
	function onHashtagAreaFocusOut(event){
		...
		var thisElem = event.currentTarget;
		var hashtagArea = $('<div>',{class:'hashtagAreaDiv w-100 h-100', id:"quizHashtags"})
		.attr('name','hashtags')[0];
		var thisForm = $(thisElem).parents('.createQuizForm')[0];
		var initText = String(thisElem.value);
	...
	$('.clickContainer').on('beatHit',(event)=>{
	...
	$('.inputNumBox').on('focusout',(event)=>{
		...
		else if(val>event.currentTarget.max)
			event.currentTarget.value = Number(event.currentTarget.max);
		else if(val<event.currentTarget.min)
			event.currentTarget.value = Number(event.currentTarget.min);
	});
	...
	$('.hashtagArea').on('focusout',(event)=>{
		...
		var theForm = $(event.currentTarget).parents('.createQuizForm')[0];
		...
		elChilds.forEach((elChild)=>{
			var innerText = elChild.innerText.trim();
			if(innerText && innerText!="")
				theForm.tags.push(innerText);
		...
		ttags.forEach((ttag)=>{
			if(ttag && ttag!="")
				theForm.tags.push(ttag.trim());
		});
		theForm.tags.forEach((ttag)=>{
			if(ttag && ttag!="")
	...
	$('#addSongBtn').on('click', async(event)=>{
		...
		var initFormDat = getFormData('.addSongForm');
		initFormDat = formDataToJSON(initFormDat);

		console.log(theForm.files);

		// initFormDat.append('userID',userID);

		initFormDat.userID = userID;
		...
		var initFormDatJSON = (initFormDat);
				...
				searchForCriteria( data.search.query ).then((res)=>{
					...
					for(let _q of res.lists){
						...
						if(isJSON(_q.quizzes)){
							_quizzes = JSON.parse(_q.quizzes); //array
						}
						let _div = $('<div>',{class:'row p-2 m-2 w-90 h-10 searchOpt activityRoute'}).append([
					...
					if(conductor){
						...
						$('.searchOpt').on('barHit',(event)=>{
							let currBar = event.detail.bar;
							if(currBar){
								let x = event.currentTarget;
								let animBox = anime({
									targets: x,
...
function createQuizSect(_quizData=null){
	...
	$(_slid[0]).load('html/home_quizCreate.html',()=>{
		loadEvents();
		var elemForm = $(_slid[0]).children('.createQuizForm')[0];
		var quizmenuctrl = $(_slid[0]).children('.quizMenuControl')[0];
		elemForm.tags = [];
		elemForm.files = [];
		// console.log(elemForm);
		$(_slid[1]).load('html/home_quizCreate_questionEdit.html',()=>{
			loadEvents();
			var elemQuestionForm = $(_slid[1]).children('.createQuestionForm')[0];
			...
			$('#containerR').load('html/home_quizCreate_quizEdit.html',()=>{
				let sliderContentSlider = setSlidableContent({
					...
					leftButton: $('<div>'),
					rightButton: $('<div>'),
					// leftButton: '#quizEditEditDetails',
					// rightButton: '#quizEditEditQuestions',
					// indexButtons: [
					// 	// {button:'#quizEditAddQuestion',index:1},
					// 	{button:'#quizEditExit',index:0},
					// ],
				});
				...
				let thisQuizMenuQuestions = setSlidableContent({
					...
					leftButton: $('<div>'),
					rightButton: $('<div>'),
					/*leftButton: _slid[1].children('.quizEditMoveBtn.moveBtnLeft'),
					rightButton: _slid[1].children('.quizEditMoveBtn.moveBtnRight'),*/
				});
				$(_slid[1]).append([
				...
				if(_quizData){
					...
					$(_slid[0]).find('.quizPassingGrade')[0].value = _quizData.passingGrade;
					...
					if(_quizData.imageURL){
						getFileBlob(`/files/images/${_quizData.imageURL}`,'image/*',(_url,_blob,_bytes)=>{
							...
							$(_slid[0]).find('.createQuizForm')[0].files = [new File([_blob], hash32(`${_url}${randomString(32)}`), {type:'image/*'})];
						});
					}
					if(_quizData.hashtags && isJSON(_quizData.hashtags)){
						var _quizHashtags = JSON.parse(_quizData.hashtags); //array
					...
					if(_quizData.questions && isJSON(_quizData.questions)){
						var _quizQuestions = JSON.parse(_quizData.questions); //array
						...
						function _addQuestion(_questionData){
							var newQuestion = $(defaultQuestionElem).clone();
							$(_slid[1]).append(newQuestion);
							thisQuizMenuQuestions.group.push(newQuestion);
							...
							getFileBlob(`/files/images/${_questionData.imageURL}`,'image/*',(_url,_blob,_bytes)=>{
								...
								$(newQuestion)[0].files = [new File([_blob], hash32(`${_url}${randomString(32)}`), {type:'image/*'})];
							});
							loadEvents();
						}
						function _addAnswer(answerDivCont,_answerData){
							...
							$(newAnswer).find('.answerCorrect')[0].checked = _answerData.correct;
							loadEvents();
						}
					}
				}
				...
				menuTracker.push({
					...
					onChange:(data)=>{
						$('#containerR').load('html/home_quizCreate_quizEdit.html',()=>{
							...
							$('#quizEditAddSong').on('click',(event)=>{
								enterSongSelect(event);
							});
							$('.songSelectConfirm').on('click',(event)=>{
								selectSongSelect(event);
							});
							$('.previewBtn').on('click',(event)=>{
								previewSongSelect(event);
							});
							$('.rewindBtn').on('click',(event)=>{
								if(conductor && data.isPlaying){
									conductor.setBeat(conductor.currBeat-10);
								}
							});
							$('.forwardBtn').on('click',(event)=>{
								if(conductor && data.isPlaying){
									conductor.setBeat(conductor.currBeat+10);
								}
							});
							...
							function selectSongSelect(event){
								...
								for(let _item of choiceItems){
									console.log(_item);
									console.log(_item);
									if($(_item).find('.formRadioBtn')[0].checked==true){
										chosenSong = _item;
									}
								}
							...
							$('#quizEditExit').on('click',function(event){
								exitQuizEdit(event);
							});
							$('#quizEditBtnCancel').on('click',function(event){
								exitQuizEdit(event);
							});
							...
							async function quizEditSave(event){
								...
								var theForm = $('.createQuizForm')[0];
								// console.log(theForm);
								console.log(theForm.tags);
								console.log(theForm.files);
								// return;
								if(!theForm.tags || !theForm.tags.length){
								...
								var tagData = JSON.stringify(theForm.tags);
								...
								initFormDat.qname = $('.quizName').val();
								initFormDat.desc = $('.quizDesc').val();
								initFormDat.userID = userID;
								initFormDat.songID = $(data.elemForm).find('.quizSongChoice').attr('data-songid');
								initFormDat.hashtags = tagData;

								if(theForm.files.length>0){
									var init_blob = theForm.files[0];
									var _blob_name = `${stringTrimToLength(theForm.files[0].name,32)}${hash32(theForm.files[0].name)}.dat`;
									var _blob = new File([init_blob],_blob_name,{type:init_blob.type});
									initFormDat.imageURL = '';
									initFormDat.imageData = [{
										name: _blob.name,
										contents: await getBase64(_blob),
										urls: [],
										type: _blob.type,
								...
								var questionsData = [];
							  	for(var q=0;q<data.quizMenuQuestions.group.length;q++){
							  		...
							  		if(qForm.files?.length>0){
										...
										initFormDat.imageData.push(null);
							  	...
							  	initFormDat.questions = questionsData;


							  	let initPassGrade = $('.quizPassingGrade')[0].value;
							  	...
							  	initFormDat.passingGrade = initPassGrade;
...
function createListSect(_listData){
	...
	$(_slid[0]).load('html/home_playlistCreate.html',()=>{
		loadEvents();
		var elemForm = $(_slid[0]).find('.createPlaylist')[0];
				...
				if(_listData){
					...
					if(_listData.imageURL){
						getFileBlob(`${_listData.imageURL}`,'image/*',(_url,_blob,_bytes)=>{
							...
							$(_slid[0]).find('.createPlaylist')[0].files = [new File([_blob], hash32(`${_url}${randomString(32)}`), {type:'image/*'})];
						});
					}
					if(_listData.quizzes){
						let _listQuizzes = JSON.parse(_listData.quizzes); //array
						...
						$('#containerR').load('html/home_playlistCreate_listEdit.html',()=>{
							...
							function addChoices(){
								...
								for(let i=0;i<choiceItems.length;i++){
									...
									var choiceBtn = $(_choiceItem).find('.formCheckBtn')[0];
							...
							async function listEditSave(event){
								...
								var initFormDat = getFormData('.createPlaylist');
								initFormDat = formDataToJSON(initFormDat);

								initFormDat.name = $('.listName').val();
								initFormDat.desc = $('.listDesc').val();
								initFormDat.userID = userID;
								...
								initFormDat.quizzes = listQuizzes;
...
function userEditSect(_userData){
	...
	$(_slid[0]).load('html/home_userEdit.html',()=>{
		var elemForm = $(_slid[0]).find('.userEditForm')[0];
		var listMenuCtrl = $(_slid[0]).find('.quizMenuControl')[0];
		elemForm.tags = [];
		elemForm.files = [];
		loadEvents();
		if(_userData){
			...
			if(_userData.imageURL){
				getFileBlob(`${_userData.imageURL}`,'image/*',(_url,_blob,_bytes)=>{
					...
					$(elemForm)[0].files = [new File([_blob], hash32(`${_url}${randomString(32)}`), {type:'image/*'})];
				});
			}
			if(_userData.preferences && isObject(_userData.preferences)){
				var _userPref = _userData.preferences;
				$(elemForm).find(`#userColor${_userPref.color}`)[0].checked = true;
				$(elemForm).find(`#userShape${_userPref.shape}`)[0].checked = true;
			}else{
				$(elemForm).find(`#userColorA`)[0].checked = true;
				$(elemForm).find(`#userShapeA`)[0].checked = true;
			}
		}
		menuTracker.push({
			element:_sliderContainer,
			...
			onChange:(data)=>{
				$('#containerR').load(`html/home_userEdit_userEdit.html`,()=>{
					...
					async function saveUserEdit(event){
						...
						initFormDat.username = $('.userUsername').val();
						initFormDat.firstname = $('.userFirstname').val();
						initFormDat.lastname = $('.userLastname').val();
						initFormDat.DOB = $('.userDOB').val();
						initFormDat.email = $('.userEmail').val();
						initFormDat.description = $('.userDesc').val();
						...
						initFormDat.preferences = (_preferences);
						...
						myHandler({
							type:'user',subType:'check',data:{username:formDat.data.username}
						},async(msg,rdata)=>{
							if(username != formDat.data.username){
								...
								await saveUserEdit_sendData(rdata);
