...
export function safeDivide(a,b, useNaN=false){
	let INF = Infinity;

	if(a == 0){
		if(b == 0){
			if(useNaN) return NaN;
			return 0;
		}else if(b == INF){
			return 0;
		}
	...
	// if(b == 0) return INF;
	// else if(b == INF||b == -INF) return 0;
	// else return (a/b);
}

export function getAverageFrom(arr){
...
export function getCSSValueInPixels(str){
	let num = 0;
	if(str.includes('px')){
		let val = Number(str.replace('px', ''));
		if(isNumber(val)){
			num = val;
		}
	}else if(str.includes('in')){
		let val = Number(str.replace('in', ''));
		if(isNumber(val)){
			num = val*96;
		}
	}else if(str.includes('cm')){
		let val = Number(str.replace('cm', ''));
		if(isNumber(val)){
			num = (val/2.54)*96;
		}
	}else if(str.includes('mm')){
		let val = Number(str.replace('mm', ''));
		if(isNumber(val)){
			num = ((val/100)/2.54)*96;
		}
	}else if(str.includes('pt')){
		let val = Number(str.replace('pt', ''));
		if(isNumber(val)){
			num = (val*72)*96;
		}
	}else if(str.includes('pc')){
		let val = Number(str.replace('pc', ''));
		if(isNumber(val)){
			num = ((val*12)*72)*96;
		}
	}
...
///AJAX///
export function ajax(jsonData, callback,url="../php/api.php",type="POST",logResults=true){
...
export async function myAjax(url="../php/api.php",type="POST",jsonData){
...
//URL PARAMS
export function processAjaxData(contentElement, urlPath="", response=null, state={}, callback=()=>{}){
...
//OTHER//
export function getNameOfClass(obj) { 
  var funcNameRegex = /function (.{1,})\(/;
  var results = (funcNameRegex).exec(obj.constructor.toString());
  return (results && results.length > 1) ? results[1] : "";
}
export function shuffleArray(arr) {
	let newArr = $.extend(new window[getNameOfClass(arr)](arr.length), arr);
  ...
  let currIndex = arr.length;
	...
	while(currIndex!=0){
		...
		currIndex--;
		[newArr[currIndex], newArr[rndIndex]] = [newArr[rndIndex], newArr[currIndex]]
	}
  return newArr;
}
		...
		if(arrItem instanceof Object && item instanceof Object){
			if(compareProperties && compareProperties instanceof Array){
				for(var pproperty of compareProperties){
					// console.log('PropA:\t'+arrItem[pproperty]+'PropI:\t'+item[pproperty]);
...
export function findItemObject(arr,item,compareProperties=null){
	var res =  findItemObjectIndex(arr,item,compareProperties);
	if(res === false) return false;
	return true;
}

export function getObjectFromArray(arr,properties){
	if(!arr || !(arr instanceof Array)) return false;
...
export function findItemObjectIndex(arr,item,compareProperties=null){
	if(!arr || !(arr instanceof Array)) return false;
	if(!item) return false;
	if(arr.length==0){
		return false;
