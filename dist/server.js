...
(async () => {
  ...
  app.post('/xapi', async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(err);
        res.status(506).send(response(false, 'WTF THE SERVER IS DEAD??', {
          status: 500
        }));
        return;
      }
  ...
  app.get('/files/:tagId', async (req, res) => {
    ...
    if (!tag) {
      res.status(404).send(response(false, 'Access Denied or no file with name or id was found.'));
      return;
    } else {
      console.log({
        tag
    ...
    try {
      ...
      console.log(err);
      res.status(506).send(response(false, 'Internal Server error. Database not connecting. Please contact the JBQ website admin.'));
      return;
    }
    try {
  ...
  app.post('/api', async (req, res) => {
    ...
    */
    ...
    if (!reqData?.data) {
      res.status(400).send(response(false, 'Error in request: No data available.'));
      return;
    }
    let result = null;
    let db = null;
      ...
      switch (reqData?.type) {
        case 'file':
          if (!reqData?.key) {
            ...
            res.status(501).send(response(false, 'Invalid API Request. Please use a valid api key and be logged in.'));
          }
          break;
        case 'signup':
          if (await checkUser(db, reqData.data, res)) {
            res.send(response(false, 'Username/Email already exists'));
            return;
          }
        ...
        case 'login':
          if (!(await checkUser(db, reqData.data, res))) {
            res.send(response(false, 'Username/Email does not exist...maybe sign up?'));
            return;
          }
        ...
        case 'activity':
          ...
          break;
        default:
          res.status(501).send(response(false, 'Invalid APIRequest. API request type is not implemented or non-existent.'));
          break;
      }
    } catch (err) {
      console.log(err);
      res.status(506).send(response(false, 'Internal Server error. Error happened while processing request. Please contact the JBQ website admin.'));
    }
  ...
  async function uploadMultipleFiles(db, data, resp) {
    let initRes = response(false, 'Processing', {
      fileData: []
    });
    if (MY.isArray(data.files)) {
      ...
      if (initRes.success) initRes.message = 'File(s) uploaded successfully';else initRes.message = 'All files failed to upload due to error in data.';
      resp.status(200);
      return initRes;
    }
  ...
  async function uploadFile(db, data, resp) {
    ...
    */
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('files').insertOne(query);
      if (res.insertedId) {
        resp.status(202);
  ...
  async function _getFile(db, data, resp, opts = null) {
    ...
    */
    ...
    try {
      let res = await db.db(dbOpts.dbName).collection('files').find(query).toArray();
      if (!res.length) {
        resp.status(404);
  ...
  async function verify(db, data, resp) {
    ...
    try {
      let res = await db.db(dbOpts.dbName).collection('apikeys').find(query).toArray();
      if (res.length) {
        resp.status(200);
        return true;
      } else {
  ...
  async function createSecretKey(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('users').updateOne(query, update);
  ...
  async function createAPIKey(db, data, sKey, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('apikeys').insertOne(query);
      if (res.insertedId) {
        resp.status(202);
  ...
  async function checkUser(db, data, resp) {
    ...
    console.log(query);
    try {
      let res = await db.db(dbOpts.dbName).collection('users').find(query).toArray();
      if (res.length) {
        return true;
      }
    } catch (err) {
  ...
  async function createUser(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('users').insertOne(query);
      if (res.insertedId) {
        ...
        if (keyCreated) {
          ...
          await db.db(dbOpts.dbName).collection('users').deleteOne({
  ...
  async function loginUser(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('users').aggregate(pipeline).toArray();
      if (res.length) {
        for (let user of res) {
  ...
  async function logoutUser(db, data, resp) {
    ...
    try {
      let res = await db.db(dbOpts.dbName).collection('users').deleteOne({
        username: query.username
      });
      if (res.deletedCount) {
        resp.status(201);
  ...
  async function editUser(db, data, resp, all = false) {
    ...
    try {
      if (all) query = {};
      let res = await db.db(dbOpts.dbName).collection('users').updateMany(query, opts);
      if (res.modifiedCount && res.matchedCount) {
        resp.status(200);
  ...
  async function deleteUser(db, data, resp, all = false) {
    ...
    try {
      ...
      if (res.deletedCount) {
        try {
          await db.db(dbOpts.dbName).collection('quizzes').deleteMany({
            userID: data?.id
          });
          await db.db(dbOpts.dbName).collection('lists').deleteMany({
            userID: data?.id
          });
          await db.db(dbOpts.dbName).collection('apikeys').deleteMany({
            userID: data?.id
          });
        } catch (err) {
          console.log(err);
  ...
  async function addScore(db, data, id, resp) {
    ...
    try {
      ...
      let currUser;
      try {
        let currUserRes = await getUser(db, {
          id: id
        }, resp, true);
        if (!currUserRes?.success) {
      ...
      let res = await db.db(dbOpts.dbName).collection('users').updateOne(query, opts);
      if (res.modifiedCount && res.matchedCount) {
        resp.status(200);
  ...
  async function addFriend(db, data, id, resp) {
    ...
    try {
      ...
      let res0 = await db.db(dbOpts.dbName).collection('users').updateOne({
        id: id
      }, {
        $set: {
      ...
      let res1 = await db.db(dbOpts.dbName).collection('users').updateOne({
        id: data?.id
      }, {
        $set: {
      ...
      if (res0.modifiedCount && res0.matchedCount && res1.modifiedCount && res1.matchedCount) {
        resp.status(200);
  ...
  async function createQuiz(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('quizzes').insertOne(query);
      if (res.insertedId) {
        resp.status(201);
  ...
  async function getQuiz(db, data, resp, all = false) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('quizzes').aggregate(pipeline).toArray();
      if (res.length && res[0]) {
        let quiz = res[0];
        if (all) quiz = res;
        resp.status(200);
  ...
  async function createList(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('lists').insertOne(query);
      if (res.insertedId) {
        resp.status(201);
  ...
  async function getList(db, data, resp, all = false) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('lists').aggregate(pipeline).toArray();
      if (res.length && res[0]) {
        let list = res[0];
        if (all) list = res;
        resp.status(200);
  ...
  async function createSong(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('songs').insertOne(query);
      if (res.insertedId) {
        resp.status(201);
  ...
  async function getSong(db, data, resp, all = false) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('songs').aggregate(pipeline).toArray();
      if (res.length && res[0]) {
        let song = res[0];
        if (all) song = res;
        console.log({
  ...
  async function setActivity(db, data, resp) {
    ...
    try {
      ...
      let res = await db.db(dbOpts.dbName).collection('activities').insertOne(query);
      if (res.insertedId) {
        resp.status(201);
  ...
  async function getActivity(db, data, resp, all = false) {
    ...
    try {
      let res = await db.db(dbOpts.dbName).collection('activities').find(query, opts).sort({
        time: 1
      }).toArray();
      if (res.length && res[0]) {
        let acts = res[0];
  ...
  const ExpressApp = {
    routes: [{
      ...
      status: 200,
      preRender: function (req, res, data) {}
    }, {
      route: "/home",
  ...
  function toObject() {
    return JSON.parse(JSON.stringify(this, (key, value) => typeof value === 'bigint' ? value.toString() : value // return everything else unchanged
    ));
  }
})();