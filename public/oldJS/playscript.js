...
import * as mod_mouse from './myMouse.js'; myMods.push(mod_mouse);
		...
		let introCard = $('<div>',{class:'introCard sectionCard h-80 h-md-90 w-90 rounded'}).append([
			...
			.append([
				$('<div>',{class:'card-header p-2'}).append([
					...
					.append([
						'by ',$('<span>',{class:'"play-quizAuthor'}).text(`${thisQuiz?.username}`)
					]),
				]),
					...
					$('<div>',{class:'centerFlexCont w-40'}).append([
						$('<div>',{class:'m-1 p-2 w-95 h-95'}).append([
							...
							$('<div>',{class:'row w-100 m-1 text-right'}).append([
								...
								.append([
									'by ',$('<span>',{class:'play-Username'}).text(`${thisQuiz?.username}`)
								])
							]),
...
function loadEvents(){
	if(conductor && conductor instanceof Conductor){
		...
		for(let i=0;i<32;i++){
			$(`.barr.step-${i+1}`).on('stepHit',(event)=>{
				if(event.detail.step%(i+1)==0) animateStepBar(event);
			});
		}
		for(let i=0;i<8;i++){
			$(`.barr.beat-${i+1}`).on('beatHit',(event)=>{
				// console.log('B');
				if(event.detail.beat%(i+1)==0) animateBeatBar(event);
			});
		}
		for(let i=0;i<32;i++){
			$(`.barr.freq-${i+1}`).on('beatHit',(event)=>{
				let K = i;
				if(event.detail.frequencyData[K]>frequency.min){
					let freqDat = event.detail.frequencyData[K];
					animateFreqBar(event,freqDat,K);
				}
			});
		}
	...
	$('#containerFooter').on('beatHit',(event)=>{
	...
	$('.barr.beat').on('beatHit',(event)=>{
		if(event.detail.beat) 
			animateBeatBar(event);
	});
	$('.barr.step').on('stepHit',(event)=>{
...
function checkQuestion(){
	if(questionSlider){
		...
		for(let an of answerDivs){
			if($(an).find('.formRadioBtn')[0].checked){
		...
		.html( createConfirmCard(isCorrect, logTxt).find('.card') );
...
function retrievePlaylist(){
	...
	myAjax('./php/api.php','POST',formDat)
	.then((data)=>{
		console.log(data);
		// return;
